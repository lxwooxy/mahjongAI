...
# ============================================================================
# GAME SIMULATION WITH NERVOUSNESS AND RESHUFFLE
# ============================================================================

"""
Simulate a turn with nervousness tracking (hand-switching allowed)
"""
function simulate_turn_nervous(game_state::GameState, game_log::Dict, nervousness::Int)
    ...
    # Make decision WITH nervousness tracking
    ...
    # Update nervousness tracking in main game state
...
"""
Simulate game with nervousness AND reshuffle (max 1 reshuffle)
Nervousness counter persists across reshuffle
"""
function simulate_game_nervous_reshuffle(all_hands::Vector{MahjongHand}, 
                                        max_total_turns::Int,
                                        nervousness::Int)
    ...
    game_state = GameState(
        ...
        Vector{Tile}[],
        [Tile[]],
        0,           # turns_since_progress
        nothing      # last_progress_value
    )
    ...
    while total_turns < max_total_turns
        result = simulate_turn_nervous(game_state, game_log, nervousness)
        ...
        elseif result == :wall_empty
            ...
            if !game_log[:reshuffled]
                ...
                # Players keep their hands
                # IMPORTANT: Reset committed hand but KEEP nervousness tracking
                # If they were 5 turns stuck before reshuffle, they still have 5 turns left
                ...
                # Continue playing (nervousness counter persists!)
                continue
            else
...
"""
Run benchmark with nervousness and reshuffle for multiple nervousness values
"""
function run_nervous_reshuffle_benchmark(n_games::Int=200; 
                                        max_turns::Int=200,
                                        nervousness_values::Vector{Int}=[10, 5, 3])
    println("\n" * "="^80)
    println("🎮 NERVOUSNESS + RESHUFFLE BENCHMARK")
    println("="^80)
    println("Rules: Hand-switching when stuck + 1 reshuffle on draw")
    println("Max total turns: $max_turns")
    println("Games per nervousness: $n_games")
    println("Nervousness values: $(join(nervousness_values, ", "))")
    println("="^80)
    ...
    # Store results for each nervousness value
    all_results = Dict{Int, Dict}()
    
    for nervousness in nervousness_values
        println("\n" * "█"^80)
        ...
        println("\nRunning simulations...")
        for i in 1:n_games
            Random.seed!(i + nervousness * 10000)  # Different seeds per nervousness
            
            game_log = simulate_game_nervous_reshuffle(all_hands, max_turns, nervousness)
        ...
        println("="^80)
        println("📊 RESULTS (Nervousness = $nervousness)")
        println("="^80)
        ...
        if !isempty(draw_games)
            println("\n📉 Draw Analysis:")
            println("   Total draws: $(length(draw_games))/$n_games ($(round(length(draw_games)/n_games*100, digits=1))%)")
            ...
            if !isempty(draw_games)
                println("   Same pattern:    $(pattern_overlaps)/$(length(draw_games)) ($(round(pattern_overlaps/length(draw_games)*100, digits=1))%)")
                println("   Similar progress: $(similar_progress)/$(length(draw_games)) ($(round(similar_progress/length(draw_games)*100, digits=1))%)")
                println("   Both stuck:      $(both_stuck)/$(length(draw_games)) ($(round(both_stuck/length(draw_games)*100, digits=1))%)")
            end
        end
    end
    ...
    for nervousness in nervousness_values
        ...
        println("  Draws:               $(results[:draw])/$n_games ($(round(results[:draw]/n_games*100, digits=1))%)")
        println("  Timeouts:            $(results[:timeout])/$n_games ($(round(results[:timeout]/n_games*100, digits=1))%)")
        println("  Reshuffles:          $(data[:games_reshuffled])/$n_games ($(round(data[:games_reshuffled]/n_games*100, digits=1))%)")
        
        if total_decisive > 0
            println("  Player wins:         $(results[:player_won])/$total_decisive decisive ($(round(results[:player_won]/total_decisive*100, digits=1))%)")
        end
    end
...
if abspath(PROGRAM_FILE) == @__FILE__
    all_results = run_nervous_reshuffle_benchmark(200, max_turns=200, nervousness_values=[10, 5, 3])
end